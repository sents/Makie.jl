...
function draw_mesh2D(scene, screen, @nospecialize(plot), @nospecialize(mesh))
    ...
    colorrange = convert_attribute(get_value(plot, :colorrange, nothing), key"colorrange"())::Union{Nothing, Vec2f}
    ...
    return draw_mesh2D(plot, screen, cols, vs, fs)
end
...
function draw_mesh3D(
    ...
    colorrange = convert_attribute(get_value(attributes, :colorrange, nothing), key"colorrange"())::Union{Nothing, Vec2f}
    matcap = get_value(attributes, :matcap, nothing)
